...
@interface WJSegmentControl ()<WJSegmentControlDelegate>{
    CGFloat witdthFloat;
    UIView * buttonDownView;
    NSInteger selectSeugment;
}
@end
@implementation WJSegmentControl
...
+(WJSegmentControl*)segmentControlFrame:(CGRect)frame btnNumbers:(NSArray *)btnNumbers backgroundColor:(UIColor *)backgroundColor titleColor:(UIColor *)titleColor titleFont:(UIFont *)titleFont selectedColor:(UIColor *)selectedColor buttonDownColor:(UIColor *)buttonDownColor delegate:(id)delegate{
    WJSegmentControl * smc = [[self alloc] initWithFrame:frame];
